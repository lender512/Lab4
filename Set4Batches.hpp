...
class Set4Batches{
        ...
        for(auto const& key : keys){
            ...
            do{
                ...
                if(current == sub_root){
                    ...
                    if(workers > 1) new_subroots.fetch_add(1);
                    break;
    ...
    void insert_t(std::vector<Key> const& keys, Pointer& current){
        if(current != null && free_workers > 1 && keys.size() > free_workers && IS_NOT_LEAF(current)){
            ...
            std::vector<int> subtrees; subtrees.reserve(SIZE(current) + 1);
            for(int i = 0; i <= SIZE(current); ++i) if(subset[i].size() > 0) subtrees.push_back(i);
            std::sort(subtrees.begin(), subtrees.end(), [&subset](int a, int b){return subset[a].size() < subset[b].size();});


            const int ideal_group_sz = keys.size() / free_workers;
            int max_subtrees_group = 0;
            int max_subtrees_group_sz = 0;
            std::vector<int> subtrees_groups; subtrees_groups.reserve(SIZE(current) + 1);
            ...
            if(free_workers == 0 && max_subtrees_group_sz < 2*ideal_group_sz){
                for(int i = 0; i<subtrees_groups.size(); ++i){
                    ...
                    threads.emplace_back([this, &subset, &subtrees, start, end, current]{
                        for(int j = start; j<end; ++j) this->insert_r(subset[subtrees[j]], CHILD(current, subtrees[j]));
                    });
                    start = end;
                }
            }else{
                for(int i = 0; i<subtrees_groups.size(); ++i){
                    ...
                    if(i != max_subtrees_group){
                        ...
                        if(start < (end - 1)){
                            threads.emplace_back([this, &subset, &subtrees, start, end, current]{
                                for(int j = start; j<(end-1); ++j) this->insert_r(subset[subtrees[j]], CHILD(current, subtrees[j]));
                            });
                        }
                        threads.emplace_back([this, &subset, &subtrees, end, current]{
                            this->insert_t(subset[subtrees[end-1]], CHILD(current, subtrees[end-1]));
                        });
                    }
                    start = end;
                }
    ...
    void contains_t(std::vector<Key> const& keys, std::vector<KeyExistence>& result, int from, Pointer current){
        if(current != null && free_workers > 1 && keys.size() > free_workers && IS_NOT_LEAF(current)){
            ...
            std::vector<std::array<int, 2>> subtrees; subtrees.reserve(SIZE(current) + 1);
            int sub_from = from;
            for(int i = 0; i <= SIZE(current); ++i) {
                if(subset[i].size() > 0) subtrees.push_back({i,sub_from});
                sub_from += subset[i].size();
            }
            std::sort(subtrees.begin(), subtrees.end(), [&subset](std::array<int, 2> a, std::array<int, 2> b){return subset[a[0]].size() < subset[b[0]].size();});
            ...
            if(free_workers == 0 && max_subtrees_group_sz < 2*ideal_group_sz){
                for(int i = 0; i<subtrees_groups.size(); ++i){
                    const int end = subtrees_groups[i];
                    threads.emplace_back([this, &subset, &subtrees, &result, start, end, current]{
                        for(int j = start; j<end; ++j) this->contains_r(subset[subtrees[j][0]], result, subtrees[j][1], CHILD(current, subtrees[j][0]));
                    });
                    start = end;
                }
            }else{
                for(int i = 0; i<subtrees_groups.size(); ++i){
                    ...
                    if(i != max_subtrees_group){
                        ...
                        if(start < (end - 1)){
                            threads.emplace_back([this, &subset, &subtrees, &result, start, end, current]{
                                for(int j = start; j<(end-1); ++j) this->contains_r(subset[subtrees[j][0]], result, subtrees[j][1], CHILD(current, subtrees[j][0]));
                            });
                        }
                        threads.emplace_back([this, &subset, &subtrees, &result, end, current]{
                            this->contains_t(subset[subtrees[end-1][0]], result, subtrees[end-1][1], CHILD(current, subtrees[end-1][0]));
                        });
                    }
                    start = end;
                }
    ...
    void remake(){
        ...
        while(IS_NOT_LEAF(current)) current = CHILD(current, 0);
    ...
    Set4Batches(): size{0}, new_lvls{0}, root{null}, new_subroots{0}{}
