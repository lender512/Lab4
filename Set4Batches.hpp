...
private:
    ...
    mid = degree / 2;
    static constexpr Size
    min = (degree / 2) + (degree % 2) - 1;
    static constexpr Size
    max_lvl = 22;
    static constexpr Size
    dirty_bitset_sz = (capacity / 32) + 32 * (capacity % 32 != 0);
    ...
    void insert_t(std::vector <Key> const &keys, Pointer &current) {
        if (IS_NOT_LEAF(current) && keys.size() > 1) {
            ...
            std::sort(jobs.begin(), jobs.end(), [&subset](Job const &a, Job const &b) {
                return subset[a.subset].size() > subset[b.subset].size();
            });
